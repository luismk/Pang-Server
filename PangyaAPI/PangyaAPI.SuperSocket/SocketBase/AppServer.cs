        ...
        where TAppSession : AppSession<TAppSession, TRequestInfo>, IAppSession, new()
    {
      ...
      public AppServer(string ServerName)
        ...
        protected void dispach_packet_same_thread(TAppSession _session, Packet _packet)
        {
            ...
            {
                _smp.Message_Pool.push("[TcpServer.dispach_packet_same_thread][ErrorSystem] " + e.Message);
            }
            if (checkPacket(_session, _packet)/*Check Packet*/)
            {
                if (func != null && func.ExecCmd(new ParamDispatch
                {
                    _session = _session,
                    _packet = _packet
                }) != 0)
        ...
        protected abstract bool checkPacket(TAppSession session, Packet packet);
